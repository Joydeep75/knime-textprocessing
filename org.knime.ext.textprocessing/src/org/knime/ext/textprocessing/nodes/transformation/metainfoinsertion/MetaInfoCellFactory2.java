...
final class MetaInfoCellFactory2 extends SingleCellFactory {
    ...
    public DataCell getCell(final DataRow row) {
        // value of new cell is value of old cell until new document has been created successfully
        DataCell newCell = row.getCell(m_docColIdx);
        if (!row.getCell(m_docColIdx).isMissing()) {
            final Document d = ((DocumentValue)row.getCell(m_docColIdx)).getDocument();
